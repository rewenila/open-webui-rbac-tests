...
from actions.knowledge import create_knowledge
from actions.models import add_model
from actions.prompts import create_prompt
...
def test_user_does_not_have_administrative_permissions(setup_data):
    _, user_auth_token, _, base_url = setup_data

    add_model_response = add_model(user_auth_token, add_model_payload(), base_url)
    assert add_model_response.status_code == 401, '[ERROR] User was allowed to add new model'

    create_knowledge_response = create_knowledge(user_auth_token, create_knowledge_payload(), base_url)
    assert create_knowledge_response.status_code == 401, '[ERROR] User was allowed to create knowledge'

    create_prompt_response = create_prompt(user_auth_token, create_prompt_payload(), base_url)
    assert create_prompt_response.status_code == 401, '[ERROR] User was allowed to create prompt'

    update_admin_config_response = update_admin_config(user_auth_token, update_admin_config_payload(False, False, 'admin', False, False), base_url)
    assert update_admin_config_response.status_code == 401, '[ERROR] User was allowed to change admin config'
